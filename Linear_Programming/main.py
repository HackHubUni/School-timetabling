...
def main():
  ...
  calendar = Calendar(asignaturas, profesores, asignatura_list_profes, grupo_asignatura_tiempo, len(dias), len(turnos),
                      aulas, grupos)
  ...
  for aula in aulas:
    ...
    for dia in dias:
      for turno in turnos:
        for profesor in profesores:
          for grupo in grupos: # se pone los grupos porque no importa si un profe en un turno en una aula le imparte a mas de un grupo
            s = sum(
              vars[profesor, asignatura, aula, grupo, turno, dia]  for aula in aulas
              for asignatura in asignaturas if asignatura in asignaturas_profes[profesor])
            model.add(s <= 1)
