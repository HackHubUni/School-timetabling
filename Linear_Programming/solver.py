...
def solver(subjects_name_list: list[str], dict_subjects_by_time: dict[str:int], teachers_names: list[str],
        ...
        for asignatura in subjects_name_list:
          for grupo in groups_names:
            s = sum(
              variables[profesor, asignatura, aula, grupo, turno, dia] for profesor in teachers_names for aula in classrooms_names
        ...
        for aula in classrooms_names:
          for dia in days:
            for turno in shifts:
              s = sum(
                variables[profesor, asignatura, aula, grupo, turno, dia] for grupo in groups_names for profesor in teachers_names
          ...
          for dia in days:
            for turno in shifts:
              for profesor in teachers_names:
                for grupo in groups_names:  # se pone los grupos porque no importa si un profe en un turno en una aula le imparte a mas de un grupo
                  s = sum(
                    variables[profesor, asignatura, aula, grupo, turno, dia] for aula in classrooms_names
                    for asignatura in subjects_name_list if asignatura in dict_teachers_to_subjects[profesor])
                  model.add(s <= 1)
